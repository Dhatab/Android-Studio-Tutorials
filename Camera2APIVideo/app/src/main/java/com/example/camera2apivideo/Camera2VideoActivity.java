package com.example.camera2apivideo;
...
public class Camera2VideoActivity extends AppCompatActivity {
    ...
    private static class CompareSizeByArea implements Comparator<Size> {
        ...
        public int compare(Size lhs, Size rhs) {
            return Long.signum((long) lhs.getWidth() * lhs.getHeight() /
                    (long) rhs.getHeight() * rhs.getHeight());
        }
    }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if(requestCode == REQUEST_CAMERA_PERMISSION_RESULT){
            if(grantResults[0] != PackageManager.PERMISSION_GRANTED){
                Toast.makeText(getApplicationContext(),"Application needs camera permission",Toast.LENGTH_SHORT).show();
            }
        }
    }

    //to make UI fullscreen
    @Override
    ...
    //background thread is to push out all task out to UI thread to so we dont lose frames
    private void startBackgroundThread(){
        mBackgroundHandlerThread = new HandlerThread("Camera2Video API");
